...
# Function to run the preprocessing script with the uploaded PDF
def run_preprocessing(pdf_path):
    st.write("Running preprocessing script. This might take a while...")
    result = subprocess.run(["python", "preprocess_bible.py", pdf_path], capture_output=True, text=True)
...
def main():
    ...
    # PDF Upload
    uploaded_pdf = st.file_uploader("Upload your Bible PDF", type="pdf")
    
    if uploaded_pdf:
        # Save the uploaded PDF to a temporary location
        pdf_path = os.path.join("uploaded_bible.pdf")
        with open(pdf_path, "wb") as f:
            f.write(uploaded_pdf.read())
        
        st.write("PDF uploaded successfully.")
        
        # Run preprocessing with the uploaded PDF
        if not os.path.exists(corpus_file) or not os.path.exists(embeddings_file):
            ...
            if not run_preprocessing(pdf_path):
                st.stop()
